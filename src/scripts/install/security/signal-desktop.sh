...
if [[ ! -f /usr/share/keyrings/signal-desktop-keyring.gpg ]]; then
    ...
    [[ -f "$TEMP_DIR/signal-key.asc" ]] && gpg --dearmor < "$TEMP_DIR/signal-key.asc" | sudo tee /usr/share/keyrings/signal-desktop-keyring.gpg >/dev/null || true
fi
...
if [[ ! -f "$signal_list" ]] || ! grep -q updates.signal.org "$signal_list" 2>/dev/null; then
    echo 'deb [arch=amd64 signed-by=/usr/share/keyrings/signal-desktop-keyring.gpg] https://updates.signal.org/desktop/apt xenial main' |         sudo tee "$signal_list" >/dev/null || true
